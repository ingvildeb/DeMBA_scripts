...
import cv2
from utils.generate_target_slice import (
    generate_target_slice,
    generate_target_coordinates,
)
import numpy as np
...
key_ages = [56, 28, 14, 4]
seg_images = glob(
    "/home/harryc/github/AllenDownload/downloaded_data/*/*/expression/*.jpg"
)
for age in tqdm(key_ages[:]):
    ...
    for a in tqdm(alignment["slices"][:]):
        seg_path = [
            i
            for i in seg_images
            if a["filename"].replace("png", "jpg").replace("_s", "_s0") in i
        ][0]
        ...
        if "markers" in a:
            non_lin_seg_small = non_linear(
                seg_small, a["height"], a["width"], a["markers"]
            )
        else:
    ...
    ccfAtlas = CCF_translator.Volume(
        atlas_volume,
        age_PND=56,
    ...
    interpolator = NearestNDInterpolator(
        points[indices_to_fit.flatten()], fill_volume[indices_to_fit]
    )
