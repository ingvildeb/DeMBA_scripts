...
for view in views:
    ...
    out = cv2.VideoWriter(f"video_volume_{view}.mp4",cv2.VideoWriter_fourcc(*'MP4V'), 6, im1.size)
    ...
    for i,j in zip(img_array, list_of_ages):
        img_text = cv2.putText(i, f"Postnatal day {j}", org, font, fontScale, color, thickness, cv2.LINE_AA)
        img_text = cv2.putText(i, f"{view} view", org2, font, fontScale, color, thickness, cv2.LINE_AA)
