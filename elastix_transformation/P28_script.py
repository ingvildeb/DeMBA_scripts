...
# Create a mask that includes the whole moving image
moving_full_mask = sitk.Image(movingImage.GetSize(), sitk.sitkUInt8)
moving_full_mask.CopyInformation(movingImage)
moving_full_mask = sitk.Cast(moving_full_mask, sitk.sitkUInt8)
moving_full_mask += 1

fixedMasks = [fixed_full_mask]
movingMasks = [moving_full_mask]
...
for fixedManualSegName, movingManualSegName in zip(fixedManualSegNameList, movingManualSegNameList):
    ...
    fixedSegMask.SetDirection(fixedImage.GetDirection())

    movingSegMask.SetOrigin(movingImage.GetOrigin())
    ...
    fixedDistanceMap.SetDirection(fixedImage.GetDirection())
    ...
    fixedImages.append(fixedManualSeg)
    movingImages.append(movingManualSeg)
    fixedMasks.append(fixedSegMask)
