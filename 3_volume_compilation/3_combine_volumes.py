...
for path in paths:
    ...
    nib.save(
        nib.Nifti1Image(volumes[:, :, :, :17], img.affine, img.header),
        f"../demo_data/{name}_P4_to_P20.nii.gz",
    )
    nib.save(
        nib.Nifti1Image(volumes[:, :, :, 17:34], img.affine, img.header),
        f"../demo_data/{name}_P21_to_P37.nii.gz",
    )
    nib.save(
        nib.Nifti1Image(volumes[:, :, :, 34:], img.affine, img.header),
        f"../demo_data/{name}_P38_to_P56.nii.gz",
    )
